...
df_iris = pd.read_csv(r"C:\Users\gladi\Projects\unit2\src\Iris.csv") 

# filtrar y convertir
df_iris = df_iris[df_iris['Species'] != 'Iris-virginica'] 
df_iris['Species'] = df_iris['Species'].map({'Iris-setosa': 0, 'Iris-versicolor': 1})

X = df_iris.drop(['Id', 'Species'], axis=1, errors='ignore').values.astype(np.float32)
y = df_iris['Species'].values.astype(np.int32)
...
print("\n--- UMBRAL 0.5 ---")
y_est_bin_50 = (y_est_prob >= 0.5).astype(int)
...
# el umbral se escogio de 0.5 y 0.3 para comparar las metricas, el de 0.5 se espera que sea mas preciso y 
# acepte menos errores,y el de 0.3 se espera que acepte mas errores pero capture mas positivos reales.
# para el de 0.5 se obtuvo lo siguiete:
# la clase 0 tiene una precisión de 0.73 y la clase 1 de 1.00, lo que significa que todas las predicciones 
# hechas como clase 1 fueron correctas. En cambio, con el umbral de 0.3 la precisión para la clase 0 es 0
# y para la clase 1 es 0.47, o sea que muchas de las predicciones positivas no corresponden a esa clase.

#github:
#dataset:https://www.kaggle.com/datasets/amineipad/iris-flowers-dataset