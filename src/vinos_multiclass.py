...
# datset de 178 muestras 13 variables y 3 clases
dataset = load_wine()
X = dataset.data.astype(np.float32)
y = dataset.target.astype(np.int32)
class_names = [str(c) for c in dataset.target_names]

# para la division del dataset se uso train 70 por ciento Val 15 por ciento Test 15 por ciento
...
# Modelo 1 con 1 capa oculta de 8 neuronas relu y salida softmax
...
plt.title("Perdida Wine")
plt.legend()
plt.subplot(1, 2, 2)
plt.plot(history.history["accuracy"], label="Train acc")
plt.plot(history.history["val_accuracy"], label="Val acc")
plt.title("Exactitud Wine")
plt.legend()
plt.tight_layout()
plt.show()
...
disp.ax_.set_title("Matriz de Confusion Wine")
plt.show()
...
plt.title('ROC-AUC Multiclase Wine')
plt.legend(fontsize='small')
plt.subplot(1, 2, 2)
for i in range(len(clases_unicas)):
    precision, recall, _ = precision_recall_curve(y_test_bin[:, i], y_prob[:, i])
    plt.plot(recall, precision, label=f'Clase {i}')
plt.title('PR-AUC Multiclase Wine')
plt.legend(fontsize='small')
plt.tight_layout()
plt.show()
...
plt.title("Desempeno segun Umbral de Decision Wine")
